...
Shadow DOM создаёт отдельное DOM-поддерево, связанное с host-элементом. Программно Shadow Root создают через `attachShadow()`, а declarative Shadow DOM из предыдущего абзаца создаётся parser’ом из `<template shadowrootmode="...">`. Обычные селекторы документа не выбирают внутренние элементы shadow tree напрямую, а стили внутри shadow tree применяются в его собственной области.
