...
`<iframe>` встраивает отдельный HTML-документ и создаёт для него вложенный browsing context со своими `window`, `document`, навигацией и загрузкой ресурсов. Для обычного tuple origin существенны scheme, host и port: два документа являются same-origin, когда эти компоненты их origins совпадают; иначе они cross-origin. Sandbox без `allow-same-origin` заставляет встроенный документ использовать opaque origin. Такой документ не получает same-origin доступ к родителю или другому содержимому только потому, что их URL при обычных условиях имели бы одинаковый origin.
