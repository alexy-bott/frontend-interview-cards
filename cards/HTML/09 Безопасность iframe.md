...
`<iframe>` встраивает отдельный HTML-документ и создаёт для него отдельный контекст просмотра (`browsing context`) со своими `window`, `document`, навигацией и загрузкой ресурсов. Для обычного tuple origin учитываются схема (`scheme`), хост (`host`) и порт (`port`): два документа считаются same-origin, когда эти компоненты совпадают; иначе они cross-origin. `sandbox` без `allow-same-origin` заставляет встроенный документ использовать непрозрачный origin (`opaque origin`). Такой документ не получает same-origin доступ к родителю или другому содержимому только потому, что без этого ограничения их URL имели бы одинаковый origin.
...
Атрибут `allow` задаёт политику контейнера (`container policy`) в рамках Permissions Policy и может делегировать или дополнительно ограничивать отдельные возможности браузера в пределах политики родителя. Он не может восстановить возможность, уже запрещённую у родителя, и не отменяет пользовательское разрешение, если его требует соответствующий API.

`referrerpolicy` управляет данными `Referer` для навигации iframe, `loading="lazy"` позволяет браузеру отложить загрузку по правилам ленивой загрузки, а понятный `title` помогает пользователям вспомогательных технологий различать встроенные документы.

Безопасность iframe работает и в обратную сторону: нужно контролировать не только возможности встроенного документа, но и то, кто имеет право встроить вашу собственную страницу. Это задаётся политикой ответа защищаемой страницы, а не `sandbox` на стороне встраивающего документа.
