...
>
> **Ответ:** Он исчезает при вводе, может быть плохо контрастным и не является постоянным названием поля. Видимый `<label>` остаётся на экране, даёт полю доступное имя и увеличивает область нажатия.

> [!followup] FormData
> **Вопрос:** Почему поле не попало в `FormData`?
>
> **Ответ:** Сначала нужно проверить `name`: без него у значения нет ключа для отправки. Поля с `disabled`, невыбранные checkbox/radio и некоторые другие неактивные элементы тоже не включаются. У нажатой submit-кнопки собственные `name` и `value`, наоборот, могут попасть в данные и показать, каким действием отправили форму.

> [!followup] Button type
> **Вопрос:** Почему кнопка внутри формы внезапно отправляет форму?
>
> **Ответ:** У `<button>` внутри формы тип по умолчанию - `submit`. Если кнопка нужна для открытия меню или добавления поля без отправки формы, нужно указать `type="button"`.

> [!followup] Client validation
> **Вопрос:** Заменяет ли нативная валидация серверную?
>
> **Ответ:** Нет. Нативная и JavaScript-валидация быстро показывают ошибки пользователю, но запрос можно отправить без интерфейса или изменить в DevTools. Сервер должен заново проверить формат, обязательность, права доступа и бизнес-правила.

> [!followup] Validation API
> **Вопрос:** Как добавить собственную ошибку к нативной валидации?
>
> **Ответ:** Вызвать у поля `setCustomValidity('Текст ошибки')`. Непустая строка делает поле невалидным, а после исправления ошибку нужно сбросить вызовом `setCustomValidity('')`. `checkValidity()` только проверяет состояние, а `reportValidity()` дополнительно просит браузер показать сообщение.

> [!followup] External submit button
> **Вопрос:** Может ли кнопка вне `<form>` отправить эту форму?
>
> **Ответ:** Да. Форме задают `id`, а кнопке - `type="submit"` и атрибут `form` со значением этого `id`. Так кнопка остаётся связанной с формой, даже если расположена вне неё в DOM, например в нижней панели модального окна.

> [!followup] disabled readonly aria-disabled
> **Вопрос:** Чем `disabled`, `readonly` и `aria-disabled` отличаются друг от друга?
>
> **Ответ:** `disabled` выключает нативный контрол и исключает его из отправки. `readonly` сохраняет фокус и отправку значения, но запрещает редактирование поддерживаемого поля. `aria-disabled="true"` только сообщает вспомогательным технологиям, что элемент недоступен: оно само не блокирует клики, клавиатуру и отправку, поэтому это поведение должен реализовать код.
