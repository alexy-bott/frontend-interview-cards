# 17 CSS preprocessors PostCSS Autoprefixer
...
> [!followup] Sass и PostCSS
> **Вопрос:** Заменяет ли PostCSS Sass?
>
> **Ответ:** Не автоматически. Sass - конкретный язык и компилятор со своей модульной системой, переменными и функциями. PostCSS - платформа для CSS-преобразований, возможности которой зависят от набора плагинов.
>
...
> [!followup] Autoprefixer
> **Вопрос:** Что именно делает Autoprefixer?
>
> **Ответ:** Он анализирует CSS и на основе Browserslist добавляет или удаляет вендорные префиксы, необходимые выбранным браузерам. Разработчик пишет стандартное непрификсованное свойство, а плагин поддерживает нужный итоговый набор.
>
...
> [!followup] Browserslist
> **Вопрос:** Почему Browserslist лучше настроить один раз для проекта?
>
> **Ответ:** Одну конфигурацию могут читать Autoprefixer, Babel и проверки совместимости. Тогда JavaScript и CSS ориентируются на один контракт поддержки браузеров, а не на разные случайные списки.
>
...
> [!followup] Нужен ли Autoprefixer
> **Вопрос:** Нужен ли Autoprefixer, если проект поддерживает только современные браузеры?
>
> **Ответ:** Это зависит от Browserslist и используемых свойств. Для многих современных возможностей префиксы уже не нужны, и Autoprefixer ничего не добавит. Однако централизованная обработка избавляет команду от ручных префиксов и автоматически изменяет результат при обновлении списка поддерживаемых браузеров.

> [!followup] Порядок обработки
> **Вопрос:** Почему порядок Sass и PostCSS имеет значение?
>
> **Ответ:** Плагин должен получить синтаксис, который умеет разбирать. Autoprefixer обычно ожидает CSS после компиляции Sass; если передать ему необработанные конструкции SCSS, сборка завершится ошибкой или плагин не увидит нужные правила. Плагины PostCSS также могут зависеть от результатов друг друга, поэтому порядок задаётся осознанно.
