# 14 Debugging CSS DevTools common issues
...
> [!followup] Computed styles
> **Вопрос:** Чем computed styles отличаются от matched rules?
>
> **Ответ:** Matched rules отвечают на вопрос «какие правила подошли элементу и какое объявление победило». Computed styles показывают итоговое вычисленное значение свойства после каскада, наследования и вычисления относительных значений.
>
...
> [!followup] Зачёркнутое правило
> **Вопрос:** Что значит зачёркнутое CSS-объявление в DevTools?
>
> **Ответ:** Обычно объявление зачёркнуто, потому что проиграло другое объявление в каскаде. DevTools отдельно показывает выключенные и невалидные объявления. У победившего значения проверяют `!important`, каскадный слой, специфичность и порядок.
>
...
> [!followup] Production-сборка
> **Вопрос:** Почему CSS ломается только в production-сборке?
>
> **Ответ:** Production-сборка может извлекать и разделять CSS по файлам, минифицировать его, удалять считающиеся неиспользуемыми классы и строить другие пути к ресурсам. Из-за этого проявляется ошибка порядка, динамический класс исчезает или шрифт и фоновое изображение получают `404`.
>
...
> [!followup] Browser support
> **Вопрос:** Как проверить, поддерживается ли CSS-свойство?
>
> **Ответ:** Проверяют таблицу совместимости MDN, требования проекта к версиям браузеров и предупреждение DevTools. Для новой возможности можно оставить базовое рабочее правило, а улучшение поместить в feature query `@supports`, который применит его только при заявленной поддержке свойства или значения.
>
