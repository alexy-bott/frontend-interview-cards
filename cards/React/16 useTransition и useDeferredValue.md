...
> [!followup]
> **Вопрос:** `useTransition` является debounce?
>
> **Ответ:** Нет. Debounce запускает работу после временной паузы и может сократить число запросов. Transition запускает обновление с меньшим приоритетом; React может начать его сразу, прервать и повторить. В поиске debounce может ограничивать частоту запросов к API, а отложенное значение сохранять отзывчивость тяжёлого списка.

> [!followup]
> **Вопрос:** Когда выбирать `useTransition`, а когда `useDeferredValue`?
>
> **Ответ:** `useTransition` выбирают, когда код контролирует setter-функцию несрочного состояния. `useDeferredValue` выбирают, когда значение уже существует или приходит как prop, а отложить нужно его использование в тяжёлом поддереве. Оба решения наиболее полезны, когда дорогое дерево способно пропустить работу при равных входных данных, например благодаря Compiler или `memo`.

> [!followup]
> **Вопрос:** Почему состояние текстового поля нельзя обновлять в transition?
>
> **Ответ:** Управляемое поле должно синхронно получить новое `value` после `onChange`, иначе ввод будет запаздывать или возвращаться к прежнему значению. Сам `query` обновляют срочно, а результаты поиска получают отложенный `query` или отдельное состояние, обновляемое в transition.

> [!followup]
> **Вопрос:** Отправляет ли `useDeferredValue` меньше запросов?
>
> **Ответ:** Нет. Он откладывает рендер React с новым значением, но не вводит временную задержку и не отменяет побочные эффекты, привязанные к исходному состоянию. Для сокращения запросов нужны debounce, отмена через AbortController, кеш и защита от устаревших ответов.

> [!followup]
> **Вопрос:** Почему transition не спасает от тяжёлой синхронной функции?
>
> **Ответ:** Пока функция непрерывно выполняется, главный поток не может обработать ввод или отрисовать следующий кадр. React умеет освобождать поток между частями собственной работы рендера, но не может прервать произвольный цикл внутри компонента. Вычисление оптимизируют, делят или выносят в Web Worker.

> [!followup]
> **Вопрос:** Как transitions взаимодействуют с Suspense?
>
> **Ответ:** Если навигация или другое обновление помечено как transition, React старается сохранить уже показанный интерфейс, пока новое дерево ожидает данные, вместо немедленной замены ближайшим `fallback`. `isPending` позволяет показать индикатор в существующем интерфейсе. Первая загрузка без прежнего содержимого всё равно показывает запасной интерфейс.

> [!followup]
> **Вопрос:** Для чего нужен второй аргумент `initialValue` у `useDeferredValue`?
>
> **Ответ:** В актуальном React можно передать начальную отложенную версию для первого рендера. React сначала вернёт `initialValue`, затем запланирует рендер с меньшим приоритетом и фактическим `value`. На последующих обновлениях этот аргумент не используется.
