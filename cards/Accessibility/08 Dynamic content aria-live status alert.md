...
> [!followup] Приоритет
> **Вопрос:** Чем `polite` отличается от `assertive`?
>
> **Ответ:** `polite` ставит сообщение в очередь и ждёт, пока скринридер закончит текущую речь. `assertive` просит объявить изменение немедленно и может прервать текущий текст. Поэтому `assertive` оставляют для редких критичных ошибок, а сохранение, загрузку и счётчики объявляют через `polite` или `status`.

> [!followup] Status и alert
> **Вопрос:** Чем `role="status"` отличается от `role="alert"`?
>
> **Ответ:** `status` сообщает несрочный результат операции и ведёт себя примерно как `aria-live="polite"`. `alert` предназначен для важного сообщения и обычно объявляется assertive. В отличие от обычной live region, новый элемент с уже заполненным `role="alert"` обычно объявляется при добавлении в DOM. Фокусировать его не требуется, но видимый текст должен оставаться доступен для повторного чтения.

> [!followup] Фокус или объявление
> **Вопрос:** Когда переносить фокус, а когда использовать live region?
>
> **Ответ:** Live region используют, когда пользователь остаётся на текущем элементе: товар добавлен в корзину, данные сохранены, список обновился. Фокус переносят, когда появился новый контекст или требуется действие: открылся dialog, показалась сводка ошибок, началась новая страница SPA. Одновременное объявление и перенос фокуса могут дать дубли.

> [!followup] Нет объявления
> **Вопрос:** Почему скринридер не объявил сообщение в React?
>
> **Ответ:** Частая причина - live region создаётся сразу с готовым текстом, быстро размонтируется или целиком заменяется другим узлом. Область лучше держать стабильной и обновлять её содержимое. Также проверяют, что она не скрыта через `display: none`, `hidden` или `aria-hidden` и что обновления не схлопнулись в одно состояние.

> [!followup] Toast
> **Вопрос:** Как сделать toast доступным?
>
> **Ответ:** Toast, или временное всплывающее уведомление, должен иметь видимый текст, жить достаточно долго и объявляться с подходящим приоритетом, обычно через `status`. Если внутри есть действие «Отменить», оно должно быть доступно с клавиатуры; при этом нельзя неожиданно перехватывать фокус у пользователя. Критичная ошибка, требующая решения, чаще заслуживает постоянного сообщения или dialog, а не исчезающего toast.

> [!followup] Полное сообщение
> **Вопрос:** Для чего нужен `aria-atomic="true"`?
>
> **Ответ:** Он просит объявлять live region целиком при изменении любой её части. Это полезно, когда отдельный изменившийся фрагмент непонятен без контекста, например в сообщении «Загружено 3 из 10». Без `aria-atomic` скринридер может объявить только новое число.

> [!followup] aria-busy
> **Вопрос:** Для чего нужен `aria-busy`?
>
> **Ответ:** `aria-busy="true"` сообщает, что область обновляется и её содержимое пока не готово. После завершения ставят `false`, чтобы вспомогательная технология получила итоговое состояние. Атрибут не показывает индикатор загрузки и не блокирует управление сам по себе; визуальное состояние и логика загрузки реализуются отдельно.
