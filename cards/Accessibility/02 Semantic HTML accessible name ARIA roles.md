...
> [!followup]
> **Вопрос:** Что такое дерево доступности, accessibility tree?
>
> **Ответ:** Дерево доступности, accessibility tree, - представление страницы, которое браузер строит для вспомогательных технологий. В нём есть роли элементов, имена, состояния и связи. Скринридер не читает исходный HTML напрямую, он работает с этой доступной моделью.

> [!followup]
> **Вопрос:** Чем `button` лучше, чем `div role="button"`?
>
> **Ответ:** `button` уже получает фокус с клавиатуры, активируется Enter и Space, поддерживает `disabled` и имеет понятную роль. `div role="button"` только сообщает роль, но не даёт полноценного поведения, поэтому его легко сделать недоступным.

> [!followup]
> **Вопрос:** Что такое доступное имя, accessible name?
>
> **Ответ:** Это основная короткая подпись элемента в дереве доступности. Например, кнопка закрытия только с иконкой может получить `aria-label="Закрыть"`; без имени скринридер объявит роль кнопки, но не объяснит действие. Доступное имя должно совпадать по смыслу с видимой подписью и оставаться различимым среди соседних элементов.

> [!followup]
> **Вопрос:** Когда использовать `aria-label`?
>
> **Ответ:** Когда элементу нужно имя, но подходящей видимой подписи действительно нет, например у кнопки только с иконкой. Если текст уже есть на странице, лучше использовать его или `aria-labelledby`. Иначе доступное имя может разойтись с видимым текстом и усложнить голосовое управление.

> [!followup]
> **Вопрос:** Чем `aria-labelledby` отличается от `aria-label`?
>
> **Ответ:** `aria-labelledby` ссылается по `id` на существующий текст и может объединить несколько элементов; такое имя автоматически меняется вместе с видимой подписью. `aria-label` хранит отдельную строку прямо на элементе. При наличии обоих для большинства ролей имя из `aria-labelledby` имеет приоритет.

> [!followup]
> **Вопрос:** Чем доступное имя отличается от `aria-describedby`?
>
> **Ответ:** Имя идентифицирует элемент: «Email», «Сохранить», «Закрыть». `aria-describedby` добавляет пояснение: требование к формату, последствие действия или текст ошибки. Длинную инструкцию не следует помещать целиком в `aria-label`, иначе название и описание смешаются.

> [!followup]
> **Вопрос:** Почему ARIA может навредить?
>
> **Ответ:** Неправильные роли, состояния или связи дают ложную информацию. `role="button"` без клавиатурного поведения обещает несуществующую кнопку, а `aria-expanded="false"` у открытого меню сообщает неверное состояние. `aria-hidden="true"` нельзя ставить на фокусируемый элемент или его родителя: пользователь сможет попасть туда фокусом, но скринридер не увидит элемент.
