...
> [!followup]
> **Вопрос:** Почему `unknown` безопаснее `any`?
>
> **Ответ:** `unknown` сохраняет проверку типов: до сужения нельзя прочитать поле, вызвать метод или передать значение функции, ожидающей конкретный тип. `any` разрешает эти действия и может распространиться по коду. На границе доверия `unknown` заставляет явно доказать форму значения.

> [!followup]
> **Вопрос:** Можно ли присвоить `unknown` любому типу?
>
> **Ответ:** Нет. Любое значение можно присвоить переменной `unknown`, но из `unknown` без проверки можно перейти только в `unknown` или `any`. Чтобы получить `string`, нужно выполнить `typeof value === "string"`; для объекта требуется type guard или схема, проверяемая во время выполнения.

> [!followup]
> **Вопрос:** Что значит, что `any` распространяется?
>
> **Ответ:** Операция над `any` часто снова возвращает `any`, а вывод generic-типа может распространить `any` на связанный результат. Например, результат `JSON.parse()` имеет `any`, поэтому цепочка обращений к полям не проверяется. Безопаснее сразу сохранить результат в `unknown` и проверить его.

> [!followup]
> **Вопрос:** Как `never` проверяет полноту `switch`?
>
> **Ответ:** После обработки всех вариантов discriminated union значение в `default` должно иметь тип `never`. Если в union добавили новый вариант и забыли новый `case`, остаток перестанет быть `never`, и TypeScript покажет ошибку в функции `assertNever` или присваивании.

> [!followup]
> **Вопрос:** Чем `void` отличается от `undefined`?
>
> **Ответ:** `undefined` является конкретным JavaScript-значением и типом этого значения. `void` в типе функции сообщает, что результат вызова не используется. Функция, переданная в параметр обратного вызова с типом `() => void`, может вернуть значение, но вызывающая сторона обязана его проигнорировать.

> [!followup]
> **Вопрос:** Почему `catch` имеет тип `unknown`?
>
> **Ответ:** В JavaScript можно бросить строку, число, объект или `Error`. При `useUnknownInCatchVariables` переменная `catch` получает `unknown`, поэтому код обязан проверить `error instanceof Error` или иначе безопасно получить сообщение. Это соответствует реальному поведению языка лучше, чем безусловный `Error`.

> [!followup]
> **Вопрос:** Где допустим `any`?
>
> **Ответ:** Как временный мост при миграции, локальный обход неверной декларации библиотеки или реализация низкоуровневого API, где безопасная поверхность проверяется отдельно. Причина должна быть понятна, область мала, а значение не должно выходить в доменный код как `any`.
...
> [!followup]
> **Вопрос:** Что произойдёт, если добавить состояние `empty`, но не изменить `switch`?
>
> **Ответ:** В ветке `default` значение `state` больше не будет `never`: там останется вариант `empty`. Вызов `assertNever(state)` вызовет ошибку TypeScript и укажет, что новый вариант не обработан.
