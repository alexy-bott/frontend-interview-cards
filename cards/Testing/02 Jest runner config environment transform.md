# 02 Jest runner config environment transform
...
> [!followup]
> **Вопрос:** Чем средство запуска тестов (test runner) отличается от библиотек проверок и mocks?
>
> **Ответ:** Test runner находит тестовые файлы, управляет их выполнением и формирует результат. Библиотека проверок (assertion library) предоставляет выражения вроде `expect(value).toBe(expected)`. Mock library создаёт управляемые замены функций и модулей.
>
...
> [!followup]
> **Вопрос:** Чем `setupFiles` отличается от `setupFilesAfterEnv`?
>
> **Ответ:** `setupFiles` выполняются после создания тестовой среды, но до установки тестового фреймворка в эту среду. Они подходят для ранней настройки переменных окружения и полифиллов, которым не нужны `expect`, `beforeEach` или `afterEach`.
>
...
> [!followup]
> **Вопрос:** Можно ли выбрать среду только для одного тестового файла?
>
> **Ответ:** Да. В начале файла можно добавить docblock `@jest-environment`, например:
>
> ```js
...
> [!followup]
> **Вопрос:** Зачем нужен `transform` и проверяет ли он типы TypeScript?
>
> **Ответ:** Трансформер переводит исходный файл в JavaScript, который понимает текущая версия Node.js и формат выполнения Jest. Например, он удаляет TypeScript-аннотации и преобразует JSX.
>
...
> [!followup]
> **Вопрос:** Почему зависимость из `node_modules` иногда вызывает `Unexpected token export`?
>
> **Ответ:** Jest по умолчанию не преобразует большинство файлов из `node_modules`, поскольку пакеты обычно уже публикуются в исполняемом формате. Ошибка возникает, если зависимость отдаёт ESM или новый синтаксис, а текущий режим Jest ожидает CommonJS либо другую версию JavaScript.
>
...
> [!followup]
> **Вопрос:** Как Jest работает с ECMAScript modules?
>
> **Ответ:** ESM - стандартная система модулей JavaScript с `import` и `export`. Для её нативного запуска Jest должен выполнять код в ESM-режиме, а трансформер должен сохранять ESM либо быть отключён. Также нужно согласовать `type: "module"`, расширения файлов и `extensionsToTreatAsEsm`.
>
...
> [!followup]
> **Вопрос:** Почему конфигурация Vite не применяется в Jest автоматически?
>
> **Ответ:** Vite и Jest имеют разные конвейеры загрузки модулей. Плагины Vite, алиасы и обработка файлов действуют при запуске Vite, но Jest не запускает этот конвейер. Поэтому эквивалентные алиасы, трансформации и замены ресурсов приходится настраивать отдельно.
>
...
> [!followup]
> **Вопрос:** Чем отличаются `clearMocks`, `resetMocks` и `restoreMocks`?
>
> **Ответ:** Эти настройки применяют соответствующее действие перед каждым тестом. `clearMocks` очищает историю вызовов, но сохраняет реализацию mock-функции. `resetMocks` также сбрасывает её реализацию к пустой. `restoreMocks` возвращает исходные реализации для spies и заменённых свойств, которые Jest умеет восстановить.
>
...
> [!followup]
> **Вопрос:** Чем `globalSetup` отличается от обычного setup-файла?
>
> **Ответ:** `globalSetup` выполняется один раз перед всеми test suites, а `globalTeardown` - один раз после них. Это подходит для дорогого внешнего ресурса, например запуска тестовой базы или сервиса. Значения, записанные в глобальную область `globalSetup`, нельзя просто читать в тестах: тесты работают в других процессах и средах.
>
...
> [!followup]
> **Вопрос:** Как искать причину ошибки конфигурации Jest?
>
> **Ответ:** Сначала определяют этап сбоя. Если тест не найден, проверяют `roots`, `testMatch` и шаблоны исключений. Если не найден импорт, проверяют путь, расширение, alias и `moduleNameMapper`. `Unexpected token` обычно указывает на формат модулей или `transform`. Отсутствующий `document` означает неверную среду, а неизвестный matcher - проблему setup-файла.
>
