...
> [!followup]
> **Вопрос:** Почему тест с `.then` может пройти, хотя проверка (assertion) внутри неверная?
>
> **Ответ:** Если callback теста не возвращает цепочку Promise, Jest видит только завершение синхронной части и помечает тест успешным. Callback `.then` выполнится позже, когда тест уже закончен.
>
...
> [!followup]
> **Вопрос:** Когда нужен `expect.assertions`?
>
> **Ответ:** Он проверяет, что в тесте выполнилось ожидаемое число assertions, то есть проверок. Это полезно в ветках callback-функций или при ручном `try/catch`, где Promise может неожиданно завершиться успешно и код с проверкой в `catch` вообще не запустится.
>
> ```ts
> expect.assertions(1);
> try {
...
> [!followup]
> **Вопрос:** Чем `findBy` отличается от `waitFor`?
>
> **Ответ:** `findBy` предназначен для появления одного DOM-элемента и возвращает его. Внутри он повторяет соответствующий `getBy`. `waitFor` принимает произвольный callback и подходит для утверждений, которые не выражаются одним поиском: изменился атрибут, завершилась анимационная стадия или mock был вызван.
>
...
> [!followup]
> **Вопрос:** Как работает повтор callback внутри `waitFor`?
>
> **Ответ:** Testing Library сначала вызывает callback сразу, затем повторяет его по интервалу и при изменениях DOM. Повтор происходит, если callback бросил ошибку. Возвращённое `false` повтор не запускает, поэтому внутри используют проверку, которая бросает ошибку при несовпадении.
>
...
> [!followup]
> **Вопрос:** Как правильно дождаться исчезновения элемента?
>
> **Ответ:** Используют `waitForElementToBeRemoved`, передав существующий элемент или функцию поиска:
>
> ```ts
...
> [!followup]
> **Вопрос:** Почему каждое действие `userEvent` нужно ожидать через `await`?
>
> **Ответ:** Реальное действие пользователя состоит из нескольких событий и может включать изменение фокуса, выделения текста и значения элемента. `userEvent` воспроизводит эту последовательность асинхронно. Без `await` проверка может выполниться между событиями и увидеть промежуточное состояние.
>
...
> [!followup]
> **Вопрос:** Чем `userEvent` отличается от `fireEvent`?
>
> **Ответ:** `fireEvent` отправляет одно указанное DOM-событие. `userEvent` моделирует пользовательское действие как последовательность событий и проверяет часть ограничений интерфейса: нельзя напечатать в disabled-поле, click меняет focus и может запускать pointer-события.
>
...
> [!followup]
> **Вопрос:** Как тестировать debounce с fake timers и `userEvent`?
>
> **Ответ:** Включают fake timers до создания `userEvent`, передают ему `advanceTimers`, выполняют ввод, затем продвигают время debounce и ожидают результат:
>
> ```ts
> jest.useFakeTimers();
...
> [!followup]
> **Вопрос:** Что означает предупреждение React `not wrapped in act(...)`?
>
> **Ответ:** `act` гарантирует, что связанные обновления React и effects обработаны до проверки. React Testing Library автоматически оборачивает свои `render`, `userEvent` и многие асинхронные helpers, поэтому вручную добавлять `act` вокруг всего теста обычно не нужно.
>
...
> [!followup]
> **Вопрос:** Нужно ли увеличивать timeout, если асинхронный тест не успевает?
>
> **Ответ:** Сначала выясняют, какое условие никогда не выполняется. Частые причины: запрос не перехвачен, действие не awaited, fake timer не продвинут, ожидается неверная accessible name или приложение действительно застряло в loading state.
>
...
> [!followup]
> **Вопрос:** Как избежать ложного успеха при проверке callback?
>
> **Ответ:** Тест должен завершаться только после вызова callback. Если API возвращает Promise, ожидают его. Для callback-style API можно использовать аргумент `done`, но нельзя одновременно возвращать Promise: Jest не сможет однозначно определить способ завершения.
>
> ```ts
