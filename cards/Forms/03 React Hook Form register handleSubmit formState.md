...
> [!followup] register
> **Вопрос:** Почему у поля должен быть `name`?
>
> **Ответ:** `name` задаёт путь значения и ошибки в объекте формы, например `user.email` или `items.0.title`. По этому пути работают `register`, `setValue`, `getValues`, `setError` и TypeScript-тип `FieldPath`. Имя должно оставаться стабильным для одного логического поля.

> [!followup] defaultValues
> **Вопрос:** Почему в RHF важны `defaultValues`?
>
> **Ответ:** Они задают начальные значения и базу, с которой RHF сравнивает `isDirty` и `dirtyFields`. Они также участвуют в `reset` и по умолчанию включаются в результат отправки. `defaultValues` кэшируются, поэтому изменение обычного prop после рендера не означает автоматическую замену значений формы.

> [!followup] formState
> **Вопрос:** Почему не стоит бездумно читать весь `formState`?
>
> **Ответ:** RHF использует подписки, чтобы обновлять только нужные части. Если компонент читает много полей `formState`, он подписывается на больше изменений и может чаще ререндериться.
>
...
> [!followup] Неуправляемые поля
> **Вопрос:** Почему RHF часто быстрее controlled форм?
>
> **Ответ:** Нативное поле, подключённое через `register`, обновляет DOM-значение без обязательного рендера родительской формы. RHF уведомляет только подписчиков значения, ошибки или другого изменившегося состояния. Преимущество заметно на больших формах, но управляемое поле само по себе не является проблемой: важен размер дерева, которое обновляется вместе с ним.

> [!followup] register vs Controller
> **Вопрос:** Когда `register` не подходит и нужен `Controller`?
>
> **Ответ:** `register` подходит нативному неуправляемому полю, которое принимает `ref`, `name`, `onChange` и `onBlur`. `Controller` или `useController` нужен для управляемого компонента с другим контрактом, например `value`/`onValueChange` у `Select` или значением `Date | null` у `DatePicker`.
>
