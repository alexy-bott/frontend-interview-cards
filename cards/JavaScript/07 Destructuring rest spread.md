...
> [!followup]
> **Вопрос:** Чем деструктуризация объекта отличается от деструктуризации массива?
>
> **Ответ:** Объектный шаблон обращается к именам свойств, поэтому порядок записи обычно не важен. Массивный шаблон использует итератор и выбирает значения по позиции. Из-за этого объект удобен для именованных параметров, а массив для короткого результата с устойчивым порядком, например пары `[state, setState]`.

> [!followup]
> **Вопрос:** Когда срабатывает значение по умолчанию в деструктуризации?
>
> **Ответ:** Только когда извлечённое значение равно `undefined`, включая отсутствующее свойство. Значения `null`, `0`, `false` и пустая строка сохраняются:
>
> ```js
...
> [!followup]
> **Вопрос:** Как безопасно деструктурировать необязательный аргумент функции?
>
> **Ответ:** Значение по умолчанию для всего параметра обрабатывает вызов без аргумента или с `undefined`:
>
> ```js
...
> [!followup]
> **Вопрос:** Чем rest-параметры отличаются от `arguments`?
>
> **Ответ:** `...args` создаёт настоящий массив, может собирать только оставшуюся часть параметров и работает в стрелочных функциях. `arguments` является массивоподобным объектом всех аргументов обычной функции и у стрелки отсутствует.
>
> Rest-параметр должен быть последним и может быть только один. Его явная запись обычно точнее показывает контракт функции.

> [!followup]
> **Вопрос:** Какие свойства копирует object spread?
>
> **Ответ:** Он копирует собственные перечисляемые строковые и символьные свойства источника. Унаследованные и неперечисляемые свойства не копируются. Геттер читается во время копирования, а в новом объекте появляется обычное свойство с полученным значением; исходный дескриптор и прототип не сохраняются.
>
> Поэтому `{ ...instance }` не является полноценным клонированием экземпляра класса.

> [!followup]
> **Вопрос:** Что произойдёт при одинаковых ключах в object spread?
>
> **Ответ:** Побеждает последнее записанное значение:
>
> ```js
...
> [!followup]
> **Вопрос:** Одинаково ли spread работает с `null`, объектом и массивом?
>
> **Ответ:** Нет. Object spread в записи `{ ...value }` пропускает `null` и `undefined` и копирует перечисляемые свойства остальных значений. Array spread `[...value]` требует итерируемый объект, поэтому `[...null]` и `[...plainObject]` выбросят `TypeError`. Строка является итерируемой, поэтому `[..."abc"]` создаст массив символов строки.

> [!followup]
> **Вопрос:** Всегда ли безопасно передавать большой массив через `fn(...items)`?
>
> **Ответ:** Нет. Движок имеет практический предел числа аргументов вызова, и очень большой массив может привести к ошибке диапазона или чрезмерному расходу памяти. Для обработки больших коллекций используют цикл, `reduce` или API, принимающий сам массив, а не миллионы отдельных аргументов.
...
> [!followup]
> **Вопрос:** Что будет выведено и почему исходный город изменится?
>
> **Ответ:** Будут выведены `1`, `"Ada"`, `"Grace"`, `"Berlin"`. Object rest собрал свойства кроме `id`, а последний `name` переопределил имя в копии. Вложенный `profile` не копировался глубоко, поэтому `copy.profile` и `user.profile` указывают на один объект.
