...
> [!followup]
> **Вопрос:** Когда начинается и заканчивается TDZ?
>
> **Ответ:** Она начинается при входе в область, где находится лексическое объявление, и заканчивается, когда выполнение достигает инициализации этой переменной. Для блока это может быть начало `{}`, для модуля начало его окружения, для параметров функции действует отдельный порядок инициализации.
>
...
> [!followup]
> **Вопрос:** Почему `typeof missing` работает, а `typeof value` в TDZ выбрасывает ошибку?
>
> **Ответ:** Для полностью необъявленного идентификатора `typeof missing` специально возвращает строку `"undefined"`. Но привязка `let` или `const` в TDZ уже существует и пока запрещена для чтения. Оператор `typeof` не обходит этот запрет и получает `ReferenceError`.

> [!followup]
> **Вопрос:** Почему function declaration можно вызвать раньше, а function expression нельзя?
>
> **Ответ:** Объявление функции инициализируется готовым объектом функции при подготовке области. В выражении `const load = function () {}` сначала создаётся переменная `load`, а значение функции присваивается только при выполнении этой строки. До неё `const` находится в TDZ; с `var` значение было бы `undefined`, и попытка вызова дала бы `TypeError`.

> [!followup]
> **Вопрос:** Поднимаются ли классы?
>
> **Ответ:** Привязка объявления класса создаётся заранее, но остаётся в TDZ до строки `class`. Поэтому создать экземпляр раньше объявления нельзя. Кроме того, тело класса выполняется в строгом режиме, а наследуемый класс нельзя использовать до инициализации базового класса.

> [!followup]
> **Вопрос:** Как TDZ проявляется в параметрах функции?
>
> **Ответ:** Параметры со значениями по умолчанию инициализируются слева направо. Более ранний параметр доступен более позднему, но не наоборот:
>
> ```js
> function ok(a, b = a) {}
...
> [!followup]
> **Вопрос:** Что будет выведено и почему программа продолжит работу после ошибок?
>
> **Ответ:** Будут выведены `undefined`, `ReferenceError`, `"function"`, `ReferenceError`. Обращения к `b` и `expression` происходят в TDZ, но ошибки перехвачены отдельными `try/catch`, поэтому следующие инструкции выполняются. Объявление `declaration` уже инициализировано функцией, а `var a` заранее содержит `undefined`.
