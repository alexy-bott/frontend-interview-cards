...
> [!followup]
> **Вопрос:** Что входит в origin?
>
> **Ответ:** Схема, хост и порт. Путь и query-параметры, то есть параметры запроса, в origin не входят. `https://example.com/a` и `https://example.com/b` имеют один origin, а `http://example.com`, `https://example.com` и `https://api.example.com` имеют разные.

> [!followup]
> **Вопрос:** Когда появляется preflight?
>
> **Ответ:** Когда cross-origin запрос не входит в безопасный список CORS: например, используется `PATCH`, `Authorization`, произвольный заголовок или JSON `Content-Type`. Браузер заранее проверяет метод и набор заголовков. Решение принимает браузер по параметрам запроса, а не frontend-разработчик вручную.

> [!followup]
> **Вопрос:** Почему `application/json` часто вызывает preflight?
>
> **Ответ:** `application/json` не входит в небольшой список CORS-safelisted значений `Content-Type`. Поэтому браузер проверяет разрешение сервера перед фактическим `POST` или другим запросом. Замена JSON на `text/plain` только ради обхода preflight ухудшает контракт и не является нормальным исправлением.

> [!followup]
> **Вопрос:** Передаются ли cookies в preflight-запросе?
>
> **Ответ:** CORS-preflight не включает учётные данные другого origin. Но сервер должен вернуть разрешения, подходящие для последующего запроса с учётными данными. Сам основной запрос затем отправит cookie только при `credentials: "include"` и подходящих атрибутах cookie.

> [!followup]
> **Вопрос:** Почему нельзя использовать `Access-Control-Allow-Origin: *` с учётными данными?
>
> **Ответ:** Браузер не предоставляет JavaScript ответ на запрос с учётными данными при значении `*`. Сервер должен указать конкретный доверенный origin и `Access-Control-Allow-Credentials: true`. Если origin выбирается динамически, его проверяют по списку разрешённых источников, а не копируют безусловно.

> [!followup]
> **Вопрос:** Зачем нужен `Access-Control-Expose-Headers`?
>
> **Ответ:** Даже после успешного CORS JavaScript по умолчанию видит только разрешённые заголовки ответа. Если приложению нужен собственный заголовок, например `X-Request-Id`, сервер перечисляет его в `Access-Control-Expose-Headers`. На работу самого HTTP-запроса это не влияет.

> [!followup]
> **Вопрос:** Защищает ли CORS от CSRF?
>
> **Ответ:** Нет. CORS может запретить атакующей странице прочитать ответ, но браузер способен отправить некоторые запросы вместе с cookies. CSRF использует сам факт выполнения действия. Защита проверяется на сервере через `SameSite`, CSRF token, проверку origin и корректную семантику методов.

> [!followup]
> **Вопрос:** Почему запрос работает в Postman, но не в браузере?
>
> **Ответ:** Postman не является браузерной страницей, ограниченной same-origin policy. Нужно открыть вкладку Network, проверить `Origin`, наличие и результат `OPTIONS`, а затем заголовки `Access-Control-Allow-*` фактического ответа. Ошибка может быть и у preflight, и у основного запроса.
