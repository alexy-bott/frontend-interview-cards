...
Fresh independence означает независимость evidence и итогового решения, а не требование абсолютной изоляции памяти, проекта или workspace. Canonical Fresh handoff не включает Primary rationale, change design, verdict, Primary-provided diff, Coordinator-side Russian Style findings/rationale или requested outcome. Общий ambient project/workspace context сам по себе не аннулирует Fresh lane: reviewer обязан не использовать candidate-specific authoring/editorial history из такого контекста как evidence и строить verdict только на current immutable candidate, active Levels 1–4, собственной Fresh history там, где она разрешена Follow-up workflow, и независимо проверенных источниках.

Если reviewer фактически прочитал или использовал previous candidate content, diff/patch, раскрывающий previous/current content, Primary/Coordinator rationale/findings/verdict, Russian Style findings/rationale/preferred wording или requested outcome до verdict, affected Fresh review считается contaminated и должен быть перезапущен. Само по себе непреднамеренное наличие parent SHA, сведений о существовании предыдущего commit или другой repository metadata без previous content/diff/rationale contamination не создаёт.

Permanent Fresh lane является logical workstream: он может optional rotate sessions через compact Fresh-owned lineage ledger, но новая chat для каждой correction не требуется.
